...
class TestInternetMessageTrailers:
    ...
    async def test_wait_trailers_returns_none_when_there_is_no_body(self):
        """With no `body` at all, returns `None`."""
        message = InternetMessage((), {})
        assert await message.wait_trailers() is None

    async def test_wait_trailers_drains_body_and_parses_it_as_trailers(self):
        ...
        assert dict(trailers) == {"X-Checksum": "abc"}

    async def test_wait_trailers_parses_multiple_field_lines(self):
        """Multiple field-lines all parse into the result."""
        body = BufferedReader.for_bytes(b"A: 1\r\nB: 2\r\n\r\n")
        message = InternetMessage((), {}, body=body)
        trailers = await message.wait_trailers()
        assert trailers is not None
        assert dict(trailers) == {"A": "1", "B": "2"}

    async def test_wait_trailers_returns_empty_for_just_the_closing_crlf(self):
        """A bare closing CRLF is a valid, empty trailer section."""
        body = BufferedReader.for_bytes(b"\r\n")
        message = InternetMessage((), {}, body=body)
        trailers = await message.wait_trailers()
        assert trailers is not None
        assert dict(trailers) == {}

    async def test_wait_trailers_raises_for_a_field_line_without_a_colon(self):
    ...
    async def test_wait_trailers_populates_trailers_in_place(self):
    ...
    async def test_wait_trailers_reads_body_at_most_once(self):
        ...
        assert dict(second) == {"X-Checksum": "abc"}
    ...
    async def test_wait_trailers_waits_for_body_to_actually_have_data(self):
...
class TestInternetMessageWriteTo:
    ...
    async def test_matches_bytes_for_a_headers_only_message(self):
    ...
    async def test_trailers_are_written_after_the_body(self):
    ...
    async def test_no_trailers_written_when_there_are_none(self):
    ...
    async def test_no_trailers_given_resolves_to_nothing_extra_once_body_is_drained(self):
...
class TestInternetMessageReadFromTuple:
    ...
    async def test_round_trips_a_headers_only_message(self):
        """A headers-only message round-trips to no body at all."""
        original = InternetMessage(("GET", "/foo", "HTTP/1.1"), {"Host": "example.com"})
        parsed = await InternetMessage.read_from((bytes(original), ENDPOINT))
        assert parsed is not None
        assert parsed.start_line == ["GET", "/foo", "HTTP/1.1"]
        assert dict(parsed.headers) == {"Host": "example.com"}
        assert parsed.body is None

    async def test_ignores_bytes_following_the_header_delimiter(self):
        """Bytes following the header delimiter never become a body."""
        data = b"GET /foo HTTP/1.1\r\nHost: example.com\r\n\r\nleftover-bytes"
        parsed = await InternetMessage.read_from((data, ENDPOINT))
        assert parsed is not None
        assert parsed.body is None

    async def test_returns_none_for_an_empty_buffer(self):
        """An empty buffer fails cleanly with `None`."""
        assert await InternetMessage.read_from((b"", ENDPOINT)) is None

    async def test_returns_none_for_a_header_line_without_a_colon(self):
        """A malformed header line (no `:`) returns `None`."""
        data = b"GET /foo HTTP/1.1\r\nNotAHeaderLine\r\n\r\n"
        assert await InternetMessage.read_from((data, ENDPOINT)) is None
...
class TestInternetMessageReadFromStream:
    ...
    async def test_body_is_the_reader_itself(self):
    ...
    async def test_caller_can_read_the_body_off_the_returned_reader(self):
        ...
        assert await message.body.readexactly(content_length) == b"hello"

    async def test_body_is_the_reader_even_with_nothing_declared_to_follow(self):
