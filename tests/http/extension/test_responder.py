...
class TestExtendedHTTPResponderOptions:
    ...
    async def test_allow_includes_the_standard_methods_with_no_extensions(self):
        """With no extensions, `Allow` matches a plain `HTTPResponder`."""
        responder = ExtendedHTTPResponder({})
        expected = await HTTPResponder({}).options(TARGET, {})
        result = await responder.options(TARGET, {})
        assert expected == result

    async def test_allow_includes_an_extensions_own_methods_too(self):
...
class TestExtendedHTTPResponderSuccessful:
    ...
    async def test_an_unknown_mandatory_extension_is_rejected(self):
        ...
        (response,) = await responder.successful(resource, request, {}, REMOTE, LOCAL)
        assert response.status == HTTPStatus.NOT_EXTENDED

    async def test_a_known_mandatory_extension_strips_the_m_prefix_and_dispatches_to_it(self):
    ...
    async def test_no_declared_extension_falls_through_to_the_base_responder(self):
    ...
    async def test_connect_still_falls_through_to_the_base_responders_handling(self):
        """`CONNECT` still falls through to the base responder's handling."""
        resource = InMemoryResource()
        responder = ExtendedHTTPResponder({TARGET: resource})
        request = HTTPRequest(HTTPMethod.CONNECT, TARGET)
        responses = await responder.successful(resource, request, {}, REMOTE, LOCAL)
        assert tuple(responses) == ()

    async def test_a_uri_identified_extension_is_matched_by_its_quoted_declaration(self):
    ...
    async def test_an_optional_declaration_does_not_mangle_an_m_prefixed_method_name(self):
