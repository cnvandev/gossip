...
class TestHTTPAccessorOptions:
    ...
    async def test_reports_the_resources_allowed_methods_with_no_body(self):
        ...
        (response,) = await accessor.access(resource, request, {}, {}, REMOTE, LOCAL)
        assert response.status == HTTPStatus.OK
        assert response.headers["Allow"] == "GET, PUT, PATCH, DELETE, HEAD, OPTIONS"
        assert response.body is None

    async def test_static_headers_win_over_resource_options_on_conflict(self):
        """A static header overrides a resource option of the same name."""
        resource = InMemoryResource()
        accessor = HTTPAccessor()
        request = HTTPRequest(HTTPMethod.OPTIONS, TARGET)
        (response,) = await accessor.access(resource, request, {}, {"Allow": "GET"}, REMOTE, LOCAL)
...
class TestHTTPAccessorHead:
    ...
    async def test_returns_metadata_with_no_body(self):
...
class TestHTTPAccessorGet:
    ...
    async def test_returns_the_resources_actual_stored_representation(self):
        ...
        (response,) = await accessor.access(resource, request, constraints, {}, REMOTE, LOCAL)
        assert response.status == HTTPStatus.OK
        assert response.body is not None
        assert await response.body.read() == b"hello"
        assert response.headers["Content-Type"] == "text/plain"

    async def test_static_headers_win_over_resource_metadata_on_conflict(self):
        """A static header overrides a resource metadata entry of the same name."""
        resource = InMemoryResource(b"hello", {"Server": "resource"})
        accessor = HTTPAccessor()
        request = HTTPRequest(HTTPMethod.GET, TARGET)
        (response,) = await accessor.access(resource, request, {}, {"Server": "static"}, REMOTE, LOCAL)
...
class TestHTTPAccessorPut:
    ...
    async def test_overwrites_the_resource_with_the_request_body(self):
        ...
        assert response.status == HTTPStatus.OK
        assert response.body is not None
        assert await response.body.read() == b"new content"
...
class TestHTTPAccessorPatch:
    ...
    async def test_updates_the_resource_and_returns_the_result(self):
...
class TestHTTPAccessorDelete:
    ...
    async def test_clears_the_resource_and_responds_no_content(self):
        ...
        assert response.body is not None
        assert await response.body.read() == b"to be deleted"
