...
class ResourceMap(Mapping[URI, ResourceCollection]):
    """A resource routing table; a map of `URI`s to the `Resources` they target.

    It internally uses a `gossip.utils.trie` to key on `URI`s along the domains
    & paths for easier search.
    """

    trie: TrieNode[TrieNode[ResourceCollection]]

    def __init__(self, data: Mapping[URI, ResourceCollection] | None = None):
        self.trie = TrieNode[TrieNode[ResourceCollection]]()
        if data is None:
            data = dict()
