...
class DNSClient:
    ...
    async def resolve_ip(self, domain: str, qtype: RecordType = RecordType.A, roots: Sequence[IPv4Address | IPv6Address] | None = None, port: int = 53) -> IPv4Address | IPv6Address:
        """Returns the IP address for a given domain."""
        response = await self.query(domain, qtype, roots=roots, port=port)
        if response is None:
            ...
            if cnames:
                return await self.resolve_ip(random.choice(cnames), qtype, roots=roots, port=port)

            raise ValueError(f"No IP retrievable for {domain}")

    async def nameserver(self, domain: str, roots: Sequence[IPv4Address | IPv6Address] | None = None, port: int = 53) -> IPv4Address | IPv6Address | None:
        ...
        root_endpoint = Endpoint(authority, port)
            ...
            if response.response_code == ResponseCode.NO_ERROR:
                if response.is_authoritative:
                    ...
                    if response.additional:
                        ...
                        return await self.nameserver(domain, ns_addresses, port=port)
                        ...
                        try:
                            ns_ip = await self.resolve_ip(authority_domain, roots=roots, port=port)
                        except ValueError:
                            log.error("No IP addresses found for authority `%s`", authority_domain)
                            return None
                        else:
                            log.debug("Found IP for authority: %s", ns_ip)
                            return await self.nameserver(domain, [ns_ip], port=port)
            else:
    ...
    async def query(self, domain: str, qtype: RecordType = RecordType.A, recursive: bool = False, roots: Sequence[IPv4Address | IPv6Address] | None = None, port: int = 53) -> DNSMessage | None:
        """Queries the DNS server of a domain for specific record types."""
        authority = await self.nameserver(domain, roots=roots, port=port)
        if authority is not None:
            log.debug("Querying nameserver `%s` for `%s`", authority, domain)
            ns_endpoint = Endpoint(authority, port)
    ...
    async def update(self, zone: str, hostname: str, ip_address: IPv4Address, ttl: timedelta | int, rtype: RecordType = RecordType.A, rclass: RecordClass = RecordClass.IN, roots: Sequence[IPv4Address | IPv6Address] | None = None, port: int = 53) -> DNSMessage | None:
        authority = await self.nameserver(zone, roots=roots, port=port)
        if authority is not None:
    ...
    async def insert(self, zone: str, hostname: str, ip_address: IPv4Address, ttl: timedelta | int, rtype: RecordType = RecordType.A, rclass: RecordClass = RecordClass.IN, roots: Sequence[IPv4Address | IPv6Address] | None = None, port: int = 53) -> DNSMessage | None:
        authority = await self.nameserver(zone, roots=roots, port=port)
        if authority is not None:
    ...
    async def delete(self, zone: str, hostname: str, rtype: RecordType = RecordType.A, rclass: RecordClass = RecordClass.IN, roots: Sequence[IPv4Address | IPv6Address] | None = None, port: int = 53) -> DNSMessage | None:
        authority = await self.nameserver(zone, roots=roots, port=port)
        if authority is not None:
